...
-- 地址表
CREATE TABLE T_DANGDANG_ADDRESS (
  ID      INTEGER PRIMARY KEY,
  NAME    VARCHAR2(30),
  ...
  TPHONE  VARCHAR2(15),
  MPHONE  VARCHAR2(11),
  USER_ID INTEGER REFERENCES T_DANGDANG_USER (ID)
);
DROP TABLE T_DANGDANG_ADDRESS;
DROP TABLE T_DANGDANG_ORDER_ITEM;
DROP TABLE T_DANGDANG_ORDER;
CREATE SEQUENCE T_DANGDANG_ADDRESS_SEQ START WITH 1010010 INCREMENT BY 1;

-- 订单表
CREATE TABLE T_DANGDANG_ORDER (
  ID          INTEGER PRIMARY KEY,
  CODE        VARCHAR2(100),
  TOTAL_PRICE NUMBER(10, 2),
  CREATE_TIME TIMESTAMP,
  STATUS      VARCHAR2(1),
  ADDRESS_ID  INTEGER REFERENCES T_DANGDANG_ADDRESS (ID),
  USER_ID     INTEGER REFERENCES T_DANGDANG_USER (ID)
);
CREATE SEQUENCE T_DANGDANG_ORDER_SEQ START WITH 10001000 INCREMENT BY 1;

-- 订单项表
CREATE TABLE T_DANGDANG_ORDER_ITEM (
  ID         INTEGER PRIMARY KEY,
  PRODUCT_ID INTEGER REFERENCES T_DANGDANG_PRODUCT (ID),
  COUNT      INTEGER,
  ORDER_ID   INTEGER REFERENCES T_DANGDANG_ORDER (ID)
);
CREATE SEQUENCE T_DANGDANG_ORDER_ITEM_SEQ START WITH 9001100 INCREMENT BY 1;
...
INSERT INTO T_DANGDANG_ADDRESS
VALUES (T_DANGDANG_ADDRESS_SEQ.nextval, 'heeh', '北京市海淀区东北旺西路中关村软件园', '100100', '010-2134423', '13844445555', 100036);
INSERT INTO T_DANGDANG_ADDRESS
VALUES (T_DANGDANG_ADDRESS_SEQ.nextval, 'mili', '北京市海淀区上地街道智城物业', '100100', '010-3454444', '13848888876', 100036);

INSERT INTO T_DANGDANG_ORDER
VALUES (T_DANGDANG_ORDER_SEQ.nextval, 'D' || T_DANGDANG_ORDER_SEQ.currval, 0, sysdate, 'y', 10010032, 100036);

INSERT INTO T_DANGDANG_ORDER_ITEM VALUES (T_DANGDANG_ORDER_ITEM_SEQ.nextval, 10010016, 3, 1001001026);
...
SELECT *
FROM T_DANGDANG_ORDER;

SELECT
  T_DANGDANG_ORDER_ITEM_SEQ.nextval,
  123,
  10,
  12
FROM dual;
...
CREATE TRIGGER increase_order_item
BEFORE INSERT --
  ON T_DANGDANG_ORDER_ITEM
FOR EACH ROW
  BEGIN
    SELECT T_DANGDANG_ORDER_ITEM_SEQ.Nextval
    INTO :NEW.ID
    FROM DUAL;
  END;

INSERT INTO T_DANGDANG_ORDER_ITEM (PRODUCT_ID, COUNT, ORDER_ID) VALUES (10010016, 30, 1001001026);

INSERT INTO T_DANGDANG_ORDER_ITEM (PRODUCT_ID, COUNT, ORDER_ID)
  SELECT 10010011, 30, 1001001026 FROM dual UNION ALL SELECT 10010011, 30, 1001001026 FROM dual